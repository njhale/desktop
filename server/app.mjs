    ...
    return new Promise((resolve, reject) => {
        ...
        app.prepare().then(() => {
            ...
            io.on("connection", (socket) => {
                console.log('observed connection event')
                io.emit("message", "connected");
                console.log('emitted connected event')
                socket.on("run", async (file, tool, args, workspace, threadID) => {
                    console.log(`run event received: ${file}, ${tool}, ${args}, ${workspace}, ${threadID}`)
                    if (runningScript) {
                        console.log("closing running script")
                        await runningScript.close();
                        console.log("running script closed")
                        runningScript = null;
                    }
                    try {
                        console.log("dismounting socket")
                        dismount(socket);
                        console.log("socket dismounted")
                        await mount(file, tool, args, workspace, socket, threadID, gptscript);
                        console.log("socket mounted")
                    } catch (e) {
                        console.error(`error mounting file: ${e}`)
                        socket.emit("error", e);
                    }
                });
            });
...
const mount = async (file, tool, args, workspace, socket, threadID, gptscript) => {
    ...
    if (!threadID || !state.chatState) {
        console.log(`about to run ${file} with ${JSON.stringify(opts)}`)

    ...
    <>
      ...
      >
        ...
        <div className="w-full justify-end items-center px-2 flex gap-2 mb-6">
          ...
          <Button
            color={updated ? 'warning' : 'primary' }
            className={updated ? 'animate-bounce' : ''}
            onClick={() => {
              restartScript();
              setUpdated(false);
            }}
      ...
      <Chat
        ...
        inputPlaceholder={!updated ? `Chat with ${placeholderName()}` : `Click "Refresh Chat" to chat with the updated version of ${placeholderName()}`}
      />
    </>
  );
};
